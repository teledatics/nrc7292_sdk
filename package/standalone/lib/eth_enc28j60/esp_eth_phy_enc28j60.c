...
enc28j60_update_link_duplex_speed(phy_enc28j60_t* enc28j60)
{
  ...
  PHY_CHECK(eth->phy_reg_read(
              eth, enc28j60->addr, ETH_PHY_PHSTAT2_REG_ADDR, &(phstat.val)) ==
              ESP_OK,
            "read PHSTAT2 failed",
            err);
  ...
  if (enc28j60->link_status != link) {
    ...
    if (link == ETH_LINK_UP) {
      ...
      PHY_CHECK(eth->on_state_changed(eth, ETH_STATE_SPEED, (void*)speed) ==
                  ESP_OK,
                "change speed failed",
                err);
      PHY_CHECK(eth->on_state_changed(eth, ETH_STATE_DUPLEX, (void*)duplex) ==
                  ESP_OK,
...
enc28j60_get_link(esp_eth_phy_t* phy)
{
  ...
  PHY_CHECK(enc28j60_update_link_duplex_speed(enc28j60) == ESP_OK,
...
enc28j60_reset(esp_eth_phy_t* phy)
{
  ...
  PHY_CHECK(eth->phy_reg_write(
              eth, enc28j60->addr, ETH_PHY_BMCR_REG_ADDR, bmcr.val) == ESP_OK,
            "write BMCR failed",
            err);
...
enc28j60_init(esp_eth_phy_t* phy)
{
  ...
  phyidr2_reg_t id2;
  PHY_CHECK(eth->phy_reg_read(
              eth, enc28j60->addr, ETH_PHY_IDR1_REG_ADDR, &(id1.val)) == ESP_OK,
            "read ID1 failed",
            err);
  PHY_CHECK(eth->phy_reg_read(
              eth, enc28j60->addr, ETH_PHY_IDR2_REG_ADDR, &(id2.val)) == ESP_OK,
            "read ID2 failed",
            err);
...
enc28j60_deinit(esp_eth_phy_t* phy)
{
  ...
  PHY_CHECK(enc28j60_pwrctl(phy, false) == ESP_OK,
