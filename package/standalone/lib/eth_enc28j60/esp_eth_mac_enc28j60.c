...
// ISR, slow polling
// #undef ENC29J60_POLLING

// fast polling
#define ENC29J60_POLLING 1

#ifdef ENC29J60_POLLING
#define ISR_CYCLE_TIME 1
#else
#define ISR_CYCLE_TIME 1000
#endif
...
#define MAC_CHECK(a, str, goto_tag, ret_value, ...)                            \
  do {                                                                         \
    if (!(a)) {                                                                \
      nrc_usr_print("%s (%d): \n" str, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
      ret = ret_value;                                                         \
      goto goto_tag;                                                           \
    }                                                                          \
  } while (0)
...
{
  int ret;
  static uint8_t spi_transfer_buf[SPI_TRANSFER_BUF_LEN];
...
/*       nrc7292 SDK hides HW so this capability is not exposed    */
...
spi_read_buf(spi_device_t* spi, int len, u8* data)
{
  ...
  u8* rx_buf = spi_transfer_buf + 3;
  u8* tx_buf = spi_transfer_buf;
  int ret;
  int idx;
  int xfer_len;
  int remaining;
...
static esp_err_t
spi_device_polling_transmit(spi_device_handle_t spi_hdl,
...
static void
enc28j60_init_spi(spi_device_handle_t* spi_hdl)
{
  spi_hdl->pin_miso = 12;
  spi_hdl->pin_mosi = 13;
  spi_hdl->pin_cs = 14;
  spi_hdl->pin_sclk = 15;
  spi_hdl->frame_bits = SPI_BIT8;
  spi_hdl->clock = 12000000;
  spi_hdl->mode = SPI_MODE0;
  spi_hdl->controller = SPI_CONTROLLER_SPI0;
  spi_hdl->bit_order = SPI_MSB_ORDER;
  spi_hdl->irq_save_flag = 0;
  spi_hdl->isr_handler = NULL;

  nrc_spi_init_cs(spi_hdl->pin_cs);
  nrc_spi_master_init(spi_hdl);
  nrc_spi_enable(spi_hdl, true);
  _delay_ms(100);
}
...
enc28j60_register_read(emac_enc28j60_t* emac, uint16_t reg_addr, uint8_t* value)
{
  ...
  if (enc28j60_reg_trans_lock(emac)) {
    ...
    MAC_CHECK(enc28j60_do_register_read(
                emac, !(reg_addr & 0xF000), reg_addr & 0xFF, value) == ESP_OK,
                     ...
                     uint32_t len)
{
  ...
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_ERDPTH, (addr & 0xFF00) >> 8) == ESP_OK,
            "write ERDPTH failed",
            out,
            ESP_FAIL);
                            ...
                            uint32_t reg_value)
{
  ...
  MAC_CHECK(enc28j60_register_read(emac, ENC28J60_MISTAT, &mii_status) ==
            ...
            ESP_FAIL);
  MAC_CHECK(
    !(mii_status & MISTAT_BUSY), "phy is busy", out, ESP_ERR_INVALID_STATE);
  ...
  MAC_CHECK(enc28j60_register_write(emac, ENC28J60_MIWRL, reg_value & 0xFF) ==
            ...
            ESP_FAIL);
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_MIWRH, (reg_value & 0xFF00) >> 8) == ESP_OK,
            "write MIWRH failed",
            out,
            ESP_FAIL);
...
enc28j60_verify_id(emac_enc28j60_t* emac)
{
  ...
  MAC_CHECK(enc28j60_register_read(
              emac, ENC28J60_EREVID, (uint8_t*)&emac->revision) == ESP_OK,
            "read EREVID failed",
            out,
            ESP_FAIL);
...
enc28j60_setup_default(emac_enc28j60_t* emac)
{
  ...
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_ERXSTL, ENC28J60_BUF_RX_START & 0xFF) == ESP_OK,
            "write ERXSTL failed",
            out,
            ESP_FAIL);
  MAC_CHECK(enc28j60_register_write(
            ...
            ESP_FAIL);
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_ERXNDL, ENC28J60_BUF_RX_END & 0xFF) == ESP_OK,
            "write ERXNDL failed",
            out,
            ESP_FAIL);
  MAC_CHECK(enc28j60_register_write(
  ...
  MAC_CHECK(enc28j60_register_write(emac, ENC28J60_ERXRDPTL, erxrdpt & 0xFF) ==
            ...
            ESP_FAIL);
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_ERXRDPTH, (erxrdpt & 0xFF00) >> 8) == ESP_OK,
  ...
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_ETXSTL, ENC28J60_BUF_TX_START & 0xFF) == ESP_OK,
            "write ETXSTL failed",
            out,
            ESP_FAIL);
  MAC_CHECK(enc28j60_register_write(
  ...
  MAC_CHECK(enc28j60_register_write(emac,
                                    ...
                                    ERXFCON_UCEN | ERXFCON_CRCEN |
                                      ERXFCON_BCEN | ERXFCON_MCEN) == ESP_OK,
...
emac_enc28j60_start(esp_eth_mac_t* mac)
{
  ...
  MAC_CHECK(enc28j60_do_bitwise_set(
              emac, ENC28J60_EIE, EIE_PKTIE | EIE_INTIE | EIE_TXERIE) == ESP_OK,
...
emac_enc28j60_set_addr(esp_eth_mac_t* mac, uint8_t* addr)
{
  ...
  MAC_CHECK(enc28j60_set_mac_addr(emac) == ESP_OK,
...
int
enc28j60_gpio_poll(emac_enc28j60_t* emac)
{
#ifdef ENC29J60_POLLING
  if (gpio_get_level(emac->int_gpio_num) == GPIO_LEVEL_LOW) {
    return 1;
  }
#endif
  return 0;
...
emac_enc28j60_task(void* arg)
{
  ...
  loop_start:
    ...
    if (ulTaskNotifyTake(pdTRUE, pdMS_TO_TICKS(ISR_CYCLE_TIME)) ==
          0 &&                           // if no notification ...
        enc28j60_gpio_poll(emac) == 0) { // ...and no interrupt asserted
      continue;                          // -> just continue to check again
    }

    // the host controller should clear the global enable bit for the interrupt
    // pin before servicing the interrupt
    MAC_CHECK_NO_RET(enc28j60_do_bitwise_clr(emac, ENC28J60_EIR, EIE_INTIE) ==
                       ESP_OK,
                     "clear EIE_INTIE failed",
                     loop_start);
    // read interrupt status
    MAC_CHECK_NO_RET(
      enc28j60_do_register_read(emac, true, ENC28J60_EIR, &status) == ESP_OK,
      "read EIR failed",
      loop_end);
    MAC_CHECK_NO_RET(
      enc28j60_do_register_read(emac, true, ENC28J60_EIE, &mask) == ESP_OK,
      "read EIE failed",
      loop_end);
    status &= mask;
    ...
    if (status == 0) {
      ...
      MAC_CHECK_NO_RET(
        enc28j60_register_read(emac, ENC28J60_EPKTCNT, &pk_counter) == ESP_OK,
        "read EPKTCNT failed",
        loop_end);
      if (pk_counter > 0) {
        status = EIR_PKTIF;
    ...
    if (status & EIR_PKTIF) {
      do {
        ...
        } else if (emac->parent.receive(&emac->parent, buffer, &length) ==
                   ESP_OK) {
    ...
    if (status & EIR_TXERIF) {
      ...
      MAC_CHECK_NO_RET(
        enc28j60_do_bitwise_set(emac, ENC28J60_ECON1, ECON1_TXRST) == ESP_OK,
        "set TXRST failed",
        loop_end);
      MAC_CHECK_NO_RET(
        enc28j60_do_bitwise_clr(emac, ENC28J60_ECON1, ECON1_TXRST) == ESP_OK,
        "clear TXRST failed",
        loop_end);

      // Clear Tx Error Interrupt Flag
      MAC_CHECK_NO_RET(
        enc28j60_do_bitwise_clr(emac, ENC28J60_EIR, EIR_TXERIF) == ESP_OK,
        "clear TXERIF failed",
        loop_end);
      ...
      if (emac->revision == ENC28J60_REV_B5 ||
        ...
        if (tx_status.late_collision) {
          ...
          MAC_CHECK_NO_RET(
            enc28j60_do_bitwise_clr(emac, ENC28J60_EIR, EIR_TXIF) == ESP_OK,
            "clear TXIF failed",
            loop_end);
          // Enable global interrupt flag and try to retransmit
          MAC_CHECK_NO_RET(
            enc28j60_do_bitwise_set(emac, ENC28J60_EIE, EIE_INTIE) == ESP_OK,
            "set INTIE failed",
            loop_end);
          MAC_CHECK_NO_RET(enc28j60_do_bitwise_set(
                             emac, ENC28J60_ECON1, ECON1_TXRTS) == ESP_OK,
                           "set TXRTS failed",
                           loop_end);
...
emac_enc28j60_set_link(esp_eth_mac_t* mac, eth_link_t link)
{
  ...
  switch (link) {
    case ETH_LINK_UP:
      MAC_CHECK(
        mac->start(mac) == ESP_OK, "enc28j60 start failed", out, ESP_FAIL);
      break;
    case ETH_LINK_DOWN:
      MAC_CHECK(
        mac->stop(mac) == ESP_OK, "enc28j60 stop failed", out, ESP_FAIL);
      break;
    default:
...
emac_enc28j60_transmit(esp_eth_mac_t* mac, uint8_t* buf, uint32_t length)
{
  ...
  /* ENC28J60 may be a bottle neck in Eth communication. Hence we need to check
   * if it is ready. */
  if (xSemaphoreTake(emac->tx_ready_sem,
                     pdMS_TO_TICKS(ENC28J60_TX_READY_TIMEOUT_MS)) == pdFALSE) {
    ESP_LOGW(TAG, "tx_ready_sem expired");
  }

  MAC_CHECK(enc28j60_do_register_read(emac, true, ENC28J60_ECON1, &econ1) ==
              ESP_OK,
            "read ECON1 failed",
            out,
            ESP_FAIL);

  MAC_CHECK(!(econ1 & ECON1_TXRTS),
            "last transmit still in progress",
            out,
            ESP_ERR_INVALID_STATE);

  /* Set the write pointer to start of transmit buffer area */
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_EWRPTL, ENC28J60_BUF_TX_START & 0xFF) == ESP_OK,
            "write EWRPTL failed",
            out,
            ESP_FAIL);

  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_EWRPTH, (ENC28J60_BUF_TX_START & 0xFF00) >> 8) ==
              ESP_OK,
            "write EWRPTH failed",
            out,
            ESP_FAIL);
  ...
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_ETXNDL, (ENC28J60_BUF_TX_START + length) & 0xFF) ==
              ESP_OK,
            "write ETXNDL failed",
            out,
            ESP_FAIL);
  ...
  MAC_CHECK(enc28j60_register_write(
              emac,
              ENC28J60_ETXNDH,
              ((ENC28J60_BUF_TX_START + length) & 0xFF00) >> 8) == ESP_OK,
            "write ETXNDH failed",
            out,
            ESP_FAIL);

  /* copy data to tx memory */
  uint8_t per_pkt_control =
    0; // MACON3 will be used to determine how the packet will be transmitted
  MAC_CHECK(enc28j60_do_memory_write(emac, &per_pkt_control, 1) == ESP_OK,
            "write packet control byte failed",
            out,
            ESP_FAIL);

  MAC_CHECK(enc28j60_do_memory_write(emac, buf, length) == ESP_OK,
            "buffer memory write failed",
            out,
            ESP_FAIL);
  ...
  MAC_CHECK(enc28j60_do_bitwise_set(emac, ENC28J60_ECON1, ECON1_TXRTS) ==
              ESP_OK,
            "set ECON1.TXRTS failed",
            out,
            ESP_FAIL);
out:
  return ret;
...
emac_enc28j60_receive(esp_eth_mac_t* mac, uint8_t* buf, uint32_t* length)
{
  ...
  __attribute__((aligned(4)))
  enc28j60_rx_header_t header; // SPI driver needs the rx buffer 4 byte align
  uint16_t status;

  memset(&header, 0, sizeof(header));

  // read packet header
  MAC_CHECK(enc28j60_read_packet(
              emac, emac->next_packet_ptr, (uint8_t*)&header, sizeof(header)) ==
              ESP_OK,
            "read header failed",
            out,
            ESP_FAIL);

  // get packet's length, address
  rx_len = header.length_low + (header.length_high << 8);

  // get status bits
  status = header.status_low + (header.status_high << 8);

  //     nrc_usr_print("[%s] next_packet_addr %d rx_len %d status 0x%X\n",
  //     __func__, header.next_packet_low + (header.next_packet_high << 8),
  //     rx_len, status);

  //     MAC_CHECK((RSV_GETBIT(status, RSV_RXOK) && rx_len < MAX_FRAMELEN), "bad
  //     rx status", out, ESP_FAIL);

  next_packet_addr = header.next_packet_low + (header.next_packet_high << 8);

  // read packet content
  MAC_CHECK(enc28j60_read_packet(emac,
                                 enc28j60_rx_packet_start(emac->next_packet_ptr,
                                                          ENC28J60_RSV_SIZE),
                                 buf,
                                 rx_len) == ESP_OK,
            "read packet content failed",
            out,
            ESP_FAIL);

  // free receive buffer space
  uint32_t erxrdpt = enc28j60_next_ptr_align_odd(
    next_packet_addr, ENC28J60_BUF_RX_START, ENC28J60_BUF_RX_END);
  MAC_CHECK(enc28j60_register_write(
              emac, ENC28J60_ERXRDPTL, (erxrdpt & 0xFF)) == ESP_OK,
            "write ERXRDPTL failed",
            out,
            ESP_FAIL);
  MAC_CHECK(enc28j60_register_write(
  ...
  MAC_CHECK(enc28j60_do_bitwise_set(emac, ENC28J60_ECON2, ECON2_PKTDEC) ==
              ESP_OK,
            "set ECON2.PKTDEC failed",
            out,
            ESP_FAIL);
  MAC_CHECK(enc28j60_register_read(emac, ENC28J60_EPKTCNT, &pk_counter) ==
              ESP_OK,
            "read EPKTCNT failed",
            out,
            ESP_FAIL);
...
emac_enc28j60_init(esp_eth_mac_t* mac)
{
  ...
  enc28j60_gpio_config(GPIO_09, false);
  ...
  /* reset enc28j60 */
  MAC_CHECK(
    enc28j60_do_reset(emac) == ESP_OK, "reset enc28j60 failed", out, ESP_FAIL);
  /* verify chip id */
  MAC_CHECK(
    enc28j60_verify_id(emac) == ESP_OK, "vefiry chip ID failed", out, ESP_FAIL);
  /* default setup of internal registers */
  MAC_CHECK(enc28j60_setup_default(emac) == ESP_OK,
            "enc28j60 default setup failed",
            out,
            ESP_FAIL);
  /* clear multicast hash table */
  MAC_CHECK(enc28j60_clear_multicast_table(emac) == ESP_OK,
            "clear multicast table failed",
            out,
            ESP_FAIL);

#ifndef ENC29J60_POLLING
  nrc_gpio_register_interrupt_handler(emac->int_gpio_num, enc28j60_isr_handler);
#endif

  return ESP_OK;
out:
  nrc_gpio_register_interrupt_handler(emac->int_gpio_num, NULL);
...
esp_eth_mac_t*
esp_eth_mac_new_enc28j60(const eth_enc28j60_config_t* enc28j60_config,
                         const eth_mac_config_t* mac_config)
{
  esp_eth_mac_t* ret = NULL;
  emac_enc28j60_t* emac = NULL;
  MAC_CHECK(
    enc28j60_config, "can't set enc28j60 specific config to null", err, NULL);
  MAC_CHECK(mac_config, "can't set mac config to null", err, NULL);
  emac = calloc(1, sizeof(emac_enc28j60_t));
  gemac = emac;
  enc28j60_init_spi(&((eth_enc28j60_config_t*)enc28j60_config)->spi_hdl);
  ...
  MAC_CHECK(enc28j60_config->int_gpio_num >= 0,
