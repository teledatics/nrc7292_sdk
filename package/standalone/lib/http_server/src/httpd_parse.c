...
static esp_err_t verify_url (http_parser *parser)
{
    ...
    V(TAG, LOG_FMT("received URI = %s"), r->uri);

    /* Make sure version is HTTP/1.1 */
    if ((parser->http_major != 1) && (parser->http_minor != 1)) {
        E(TAG, LOG_FMT("unsupported HTTP version = %d.%d"),
    ...
    if (http_parser_parse_url(r->uri, strlen(r->uri),
        ...
        E(TAG, LOG_FMT("http_parser_parse_url failed with errno = %d"),
                              parser->http_errno);
                        ...
                        const char *at, size_t length)
{
    ...
    V(TAG, LOG_FMT("processing url = %.*s"), length, at);
...
static esp_err_t cb_header_field(http_parser *parser, const char *at, size_t length)
{
    ...
    V(TAG, LOG_FMT("processing field = %.*s"), length, at);
...
static esp_err_t cb_header_value(http_parser *parser, const char *at, size_t length)
{
    ...
    V(TAG, LOG_FMT("processing value = %.*s"), length, at);
...
static esp_err_t cb_headers_complete(http_parser *parser)
{
    ...
    if (parser->upgrade) {
        ...
        if (httpd_req_get_hdr_value_str(r, "Upgrade", ws_upgrade_hdr_val, sizeof(ws_upgrade_hdr_val)) != ESP_OK) {
            E(TAG, LOG_FMT("Upgrade header does not match the length of \"websocket\""));
            parser_data->error = HTTPD_400_BAD_REQUEST;
            parser_data->status = PARSING_FAILED;
            return ESP_FAIL;
        }
        ...
        if (strcasecmp("websocket", ws_upgrade_hdr_val) != 0) {
            E(TAG, LOG_FMT("Upgrade header found but it's %s"), ws_upgrade_hdr_val);
            parser_data->error = HTTPD_400_BAD_REQUEST;
            parser_data->status = PARSING_FAILED;
            return ESP_FAIL;
        }
...
static esp_err_t cb_no_body(http_parser *parser)
{
    ...
    parser_data->last.length = 0;
    parser_data->status      = PARSING_COMPLETE;
    V(TAG, LOG_FMT("message complete"));
    return ESP_OK;
}
...
static void parse_init(httpd_req_t *r, http_parser *parser, parser_data_t *data)
{
    ...
    data->settings.on_url              = (http_data_cb) cb_url;
    data->settings.on_header_field     = (http_data_cb) cb_header_field;
    data->settings.on_header_value     = (http_data_cb) cb_header_value;
    data->settings.on_headers_complete = (http_cb) cb_headers_complete;
    data->settings.on_body             = (http_data_cb) cb_on_body;
    data->settings.on_message_complete = (http_cb) cb_no_body;
}
...
esp_err_t httpd_req_new(struct httpd_data *hd, struct sock_db *sd)
{
    ...
    /* Handle WebSocket */
    V(TAG, LOG_FMT("New request, has WS? %s, sd->ws_handler valid? %s, sd->ws_close? %s"),
    ...
    if (sd->ws_handshake_done && sd->ws_handler != NULL) {
        ...
        if (ra->ws_type == HTTPD_WS_TYPE_CLOSE) {
            ...
            V(TAG, LOG_FMT("Received PONG frame"));
        }
...
esp_err_t httpd_query_key_value(const char *qry_str, const char *key, char *val, size_t val_size)
{
    ...
    V(TAG, LOG_FMT("key %s not found"), key);
