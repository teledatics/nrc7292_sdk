...
static int wpa_supplicant_ctrl_iface_scan_ssid(
		struct wpa_supplicant *wpa_s, char *cmd)
{
	int id, enable;
	char *val;
	...
	*val++ = '\0';
	...
	wpa_printf(MSG_DEBUG,
			"CTRL_IFACE: SCAN_SSID id=%d enable=%d", id, enable);
	...
	if (enable < 0 || enable > 1) {
		wpa_printf(MSG_DEBUG, "scan_ssid value(%d) should be 0 or 1", enable);
		return -1;
	}
	ssid->scan_ssid = enable;

	return 0;
}

static int wpa_supplicant_ctrl_iface_ndp_preq(
		struct wpa_supplicant *wpa_s, char *cmd)
{
	int enable;
	enable = atoi(cmd);
	...
	wpa_printf(MSG_DEBUG,
			"CTRL_IFACE: NDP_PREQ enable=%d", enable);

	if (enable < 0 || enable > 1) {
		wpa_printf(MSG_DEBUG, "ndp_preq value(%d) should be 0 or 1", enable);
		return -1;
	}

	return wpa_drv_set_ndp_preq(wpa_s, enable);
}
