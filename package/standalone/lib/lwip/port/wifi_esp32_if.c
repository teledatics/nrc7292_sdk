...
//	print_buffer(buffer, length);
	...
	if ((buffer == NULL) || !netif_is_up(&wifi_esp32_netif)) {
		if (buffer) {
			free(buffer);
		}
		return NRC_FAIL;
	...
	switch (htons(ethhdr->type)) {
		...
		case ETHTYPE_IP:
			if (eth_copy_buffer_to_pbuf(buffer, length, &p) == NRC_FAIL) {
				...
				wifi_esp32_netif.input(p, &wifi_esp32_netif);
			}
			break;
...
static nrc_err_t eth_linkup_handler(esp_eth_handle_t eth_handle)
{
	...
	netif_set_link_up(&wifi_esp32_netif);

	if (eth_mode == NRC_ETH_MODE_AP) {
		if (network_mode == NRC_NETWORK_MODE_BRIDGE) {
			dhcp_start(&br_netif);
		} else {
			dhcp_start(&wifi_esp32_netif);
		}
...
static nrc_err_t eth_linkdown_handler(esp_eth_handle_t eth_handle)
{
	...
	if (network_mode == NRC_NETWORK_MODE_BRIDGE) {
		...
		if (eth_dhcp_started) {
			dhcp_stop(&wifi_esp32_netif);
			dhcp_cleanup(&wifi_esp32_netif);
		}
	}
	netif_set_link_down(&wifi_esp32_netif);
	return NRC_SUCCESS;
}
