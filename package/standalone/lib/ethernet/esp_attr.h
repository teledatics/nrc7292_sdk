...
#define COREDUMP_RTC_FAST_ATTR                                                 \
  _SECTION_ATTR_IMPL(".rtc.fast.coredump", __COUNTER__)
...
#define FLAG_ATTR_IMPL(TYPE, INT_TYPE)                                         \
  FORCE_INLINE_ATTR constexpr TYPE operator~(TYPE a)                           \
  {                                                                            \
    return (TYPE) ~(INT_TYPE)a;                                                \
  }                                                                            \
  FORCE_INLINE_ATTR constexpr TYPE operator|(TYPE a, TYPE b)                   \
  {                                                                            \
    return (TYPE)((INT_TYPE)a | (INT_TYPE)b);                                  \
  }                                                                            \
  FORCE_INLINE_ATTR constexpr TYPE operator&(TYPE a, TYPE b)                   \
  {                                                                            \
    return (TYPE)((INT_TYPE)a & (INT_TYPE)b);                                  \
  ...
  FORCE_INLINE_ATTR constexpr TYPE operator>>(TYPE a, int b)                   \
  {                                                                            \
...
// errors if two variables in the same source file have different linkage (maybe
// const & non-const) but are placed in the same custom section
//
...
#define _SECTION_ATTR_IMPL(SECTION, COUNTER)                                   \
  __attribute__((section(SECTION "." _COUNTER_STRINGIFY(COUNTER))))
