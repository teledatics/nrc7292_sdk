 ...
 * - Maximum / Minimum value of HW, including: buffer/fifo size, length of
 * transaction, frequency supported, etc.
 *
 ...
 *
 * The code depending on this file uses these boolean definitions as `#if x` or
 * `#if !x`.
 * (`#ifdef/ifndef x` is not compatible with `#define x 0`. Though we don't
 * suggest to use `#define x 0`, it's still a risk.)
 *
 * ECO & exceptions:
 * For ECO-ed booleans, `#define x "Not determined"` for them. This will cause
 * error when used by
...
// Define warning strings here for ECO-ed features to show error when they are
// used without being defined correctly
...
#define SOC_ADC_PATT_LEN_MAX                                                   \
  (16) // Two pattern table, each contains 16 items. Each item takes 1 byte. But
       // only support ADC1 using DMA mode
#define SOC_ADC_DIGI_MIN_BITWIDTH (9)
#define SOC_ADC_DIGI_MAX_BITWIDTH (12)
/*!< F_sample = F_digi_con / 2 / interval. F_digi_con = 5M for now. 30 <=
 * interva <= 4095 */
...
// SOC_GPIO_SUPPORT_RTC_INDEPENDENT not defined. On ESP32 those PADs which have
// RTC functions must set pullup/down/capability via RTC register. On ESP32-S2,
// Digital IOs have their own registers to control pullup/down/capability,
// independent with RTC registers.

// 0~39 except from 24, 28~31 are valid
#define SOC_GPIO_VALID_GPIO_MASK                                               \
  (0xFFFFFFFFFFULL & ~(0ULL | BIT24 | BIT28 | BIT29 | BIT30 | BIT31))
// GPIO >= 34 are input only
#define SOC_GPIO_VALID_OUTPUT_GPIO_MASK                                        \
  (SOC_GPIO_VALID_GPIO_MASK &                                                  \
   ~(0ULL | BIT34 | BIT35 | BIT36 | BIT37 | BIT38 | BIT39))
...
#define SOC_APLL_MIN_HZ                                                        \
  (5303031) // 5.303031 MHz, refer to 'periph_rtc_apll_freq_set' for the
            // calculation
#define SOC_APLL_MAX_HZ                                                        \
  (125000000) // 125MHz, refer to 'periph_rtc_apll_freq_set' for the calculation
...
#define SOC_I2S_TRANS_SIZE_ALIGN_WORD                                          \
  (1) // I2S DMA transfer size must be aligned to word
#define SOC_I2S_LCD_I80_VARIANT                                                \
  (1) // I2S has a special LCD mode that can generate Intel 8080 TX timing
...
/*-------------------------- MCPWM CAPS --------------------------------------*/
#define SOC_MCPWM_GROUPS                                                       \
  (2) ///< 2 MCPWM groups on the chip (i.e., the number of independent MCPWM
      ///< peripherals)
#define SOC_MCPWM_TIMERS_PER_GROUP                                             \
  (3) ///< The number of timers that each group has
#define SOC_MCPWM_OPERATORS_PER_GROUP                                          \
  (3) ///< The number of operators that each group has
#define SOC_MCPWM_COMPARATORS_PER_OPERATOR                                     \
  (2) ///< The number of comparators that each operator has
#define SOC_MCPWM_GENERATORS_PER_OPERATOR                                      \
  (2) ///< The number of generators that each operator has
#define SOC_MCPWM_TRIGGERS_PER_OPERATOR                                        \
  (2) ///< The number of triggers that each operator has
#define SOC_MCPWM_GPIO_FAULTS_PER_GROUP                                        \
  (3) ///< The number of GPIO fault signals that each group has
#define SOC_MCPWM_CAPTURE_TIMERS_PER_GROUP                                     \
  (1) ///< The number of capture timers that each group has
#define SOC_MCPWM_CAPTURE_CHANNELS_PER_TIMER                                   \
  (3) ///< The number of capture channels that each capture timer has
#define SOC_MCPWM_GPIO_SYNCHROS_PER_GROUP                                      \
  (3) ///< The number of GPIO synchros that each group has
...
#define SOC_RMT_TX_CANDIDATES_PER_GROUP                                        \
  (8) /*!< Number of channels that capable of Transmit in each group */
#define SOC_RMT_RX_CANDIDATES_PER_GROUP                                        \
  (8) /*!< Number of channels that capable of Receive in each group */
#define SOC_RMT_CHANNELS_PER_GROUP (8) /*!< Total 8 channels */
#define SOC_RMT_MEM_WORDS_PER_CHANNEL                                          \
  (64) /*!< Each channel owns 64 words memory */
#define SOC_RMT_SUPPORT_REF_TICK                                               \
  (1) /*!< Support set REF_TICK as the RMT clock source */
#define SOC_RMT_CHANNEL_CLK_INDEPENDENT                                        \
  (1) /*!< Can select different source clock for each channel */
...
/*-------------------------- SPI CAPS ----------------------------------------*/
#define SOC_SPI_HD_BOTH_INOUT_SUPPORTED                                        \
  1 // Support enabling MOSI and MISO phases together under Halfduplex mode
#define SOC_SPI_AS_CS_SUPPORTED                                                \
  1 // Support to toggle the CS while the clock toggles
...
#define SOC_SPI_PERIPH_SUPPORT_MULTILINE_MODE(spi_host)                        \
  ({                                                                           \
    (void)spi_host;                                                            \
    1;                                                                         \
  })
...
#define SOC_TOUCH_PAD_MEASURE_WAIT_MAX                                         \
  (0xFF) /*!<The timer frequency is 8Mhz, the max value is 0xff */
#define SOC_TOUCH_PAD_THRESHOLD_MAX                                            \
  (0) /*!<If set touch threshold max value, The touch sensor can't be in       \
         touched status */
...
#define SOC_UART_SUPPORT_REF_TICK                                              \
  (1)                           /*!< Support REF_TICK as the clock source */
...
#define SOC_PM_SUPPORT_TOUCH_SENSOR_WAKEUP                                     \
  (1) /*!<Supports waking up from touch pad trigger */
