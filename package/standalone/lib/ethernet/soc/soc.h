...
#define SOC_MAX_CONTIGUOUS_RAM_SIZE                                            \
  (SOC_EXTRAM_DATA_HIGH -                                                      \
   SOC_EXTRAM_DATA_LOW) ///< Largest span of contiguous memory (DRAM or IRAM) in
                        ///< the address space
...
#define VALUE_SET_FIELD(_r, _f, _v)                                            \
  ((_r) = (((_r) & ~((_f) << (_f##_S))) | ((_v) << (_f##_S))))

// set field value to a variable, used when _f is left shifted by _f##_S
#define VALUE_SET_FIELD2(_r, _f, _v)                                           \
  ((_r) = (((_r) & ~(_f)) | ((_v) << (_f##_S))))

// generate a value from a field value, used when _f is not left shifted by
// _f##_S
...
#define SET_PERI_REG_BITS(reg, bit_map, value, shift)                          \
  ({                                                                           \
    (WRITE_PERI_REG((reg),                                                     \
                    (READ_PERI_REG(reg) & (~((bit_map) << (shift)))) |         \
                      (((value)&bit_map) << (shift))));                        \
  })
...
// First and last words of the D/IRAM region, for both the DRAM address as well
// as the IRAM alias.
...
// Region of memory accessible via DMA in internal memory. See
// esp_ptr_dma_capable().
#define SOC_DMA_LOW 0x3FFB0000
#define SOC_DMA_HIGH 0x40000000

// Region of memory accessible via DMA in external memory. See
// esp_ptr_dma_ext_capable().
...
// Region of memory that is internal, as in on the same silicon die as the ESP32
// CPUs (excluding RTC data region, that's checked separately.) See
// esp_ptr_internal().
...
/*************************************************************************************************************
 *      Intr num                Level           Type                    PRO CPU
 *usage 0                       1               extern level            WMAC 1
 *1               extern level            BT/BLE Host HCI DMA 2 1 extern level
 *      3                       1               extern level
 *      4                       1               extern level            WBB
 *      5                       1               extern level            BT/BLE
 *Controller 6                       1               timer FreeRTOS Tick(L1) 7
 *1               software                BT/BLE VHCI 8                       1
 *extern level            BT/BLE BB(RX/TX) 9                       1 extern
 *level 10                      1               extern edge 11 3 profiling 12 1
 *extern level 13                      1               extern level 14 7 nmi
 *Reserved 15                      3               timer FreeRTOS Tick(L3) 16 5
 *timer 17                      1               extern level 18 1 extern level
 *      19                      2               extern level
 *      20                      2               extern level
 *      21                      2               extern level
 *      22                      3               extern edge
...
// CPU0 Interrupt number used in ROM code only when module init function called,
