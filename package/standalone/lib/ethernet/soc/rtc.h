...
extern "C"
{
   ...
   *
   * Functions in this file facilitate configuration of ESP32's RTC_CNTL
   * peripheral. RTC_CNTL peripheral handles many functions:
...
#define RTC_CLK_CAL_FRACT                                                      \
  19 //!< Number of fractional bits in values returned by rtc_clk_cal
...
#define RTC_VDDSDIO_TIEH_3_3V 1 //!< TIEH field value for 3.3V VDDSDIO
  ...
  typedef struct
  {
    ...
    uint32_t clk_8m_clk_div : 3; //!< RTC 8M clock divider (division is by
                                 //!< clk_8m_div+1, i.e. 0 means 8MHz frequency)
    uint32_t slow_clk_dcap : 8;  //!< RTC 90k clock adjustment parameter (higher
                                 //!< value leads to lower frequency)
    uint32_t clk_8m_dfreq : 8;   //!< RTC 8m clock adjustment parameter (higher
                                 //!< value leads to higher frequency)
  } rtc_clk_config_t;
...
#define RTC_CLK_CONFIG_DEFAULT()                                               \
  {                                                                            \
    .xtal_freq = RTC_XTAL_FREQ_40M, .cpu_freq_mhz = 80,                        \
    .fast_freq = RTC_FAST_FREQ_8M, .slow_freq = RTC_SLOW_FREQ_RTC,             \
    .clk_rtc_clk_div = 0, .clk_8m_clk_div = 0,                                 \
...
#define X32K_BOOTSTRAP_CONFIG_DEFAULT()                                        \
  {                                                                            \
    .x32k_cfg = X32K_CONFIG_BOOTSTRAP_DEFAULT(), .bt_lpck_div_num = 2441,      \
    .bt_lpck_div_a = 32, .bt_lpck_div_b = 13,                                  \
  }
#endif
...
#define RTC_TIME_CAL_CYCLING_MODE 1
   ...
   *              If 0, no square wave will be used to bootstrap crystal
   * oscillation.
   */
   ...
   * apll_freq = xtal_freq * (4 + sdm2 + sdm1/256 + sdm0/65536)/((o_div + 2) *
   * 2)
   *
   ...
   *      - else Sucess
   */
  uint32_t rtc_clk_apll_coeff_calc(uint32_t freq,
                                   uint32_t* _o_div,
                                   uint32_t* _sdm0,
                                   uint32_t* _sdm1,
                                   uint32_t* _sdm2);
   ...
   * @param sdm2  frequency adjustment parameter, 0..63
   */
  void rtc_clk_apll_coeff_set(uint32_t o_div,
                              uint32_t sdm0,
                              uint32_t sdm1,
                              uint32_t sdm2);
   ...
   */
  bool rtc_clk_cpu_freq_mhz_to_config(uint32_t freq_mhz,
   ...
   *
   * @note This function in not intended to be called by applications in
   * FreeRTOS environment. This is because it does not adjust various timers
   * based on the new CPU frequency.
   *
   ...
   * Assumes that XTAL frequency has been determined — don't call in startup
   * code.
   */
   ...
   * this function to update the value of APB frequency stored in
   * RTC_APB_FREQ_REG (one of RTC general purpose retention registers). This
   * should not normally be called from application code.
   *
   ...
   * @return The APB frequency value as last set via rtc_clk_apb_freq_update(),
   * in Hz.
   */
  uint32_t rtc_clk_apb_freq_get(void);

  uint32_t rtc_clk_cal_internal(rtc_cal_sel_t cal_clk,
   ...
   * incorrect loading capacitance, board design issue, or lack of 32 XTAL on
   * board).
   *
   * @param cal_clk  clock to be measured
   * @param slow_clk_cycles  number of slow clock cycles to average
   * @return average slow clock period in microseconds, Q13.19 fixed point
   * format, or 0 if calibration has timed out
   */
   ...
   *                         fixed point format (as returned by
   * rtc_slowck_cali).
   ...
   * cycles. Counter value is not writable by software. The value is not
   * adjusted when switching to a different RTC_SLOW_CLK source.
   *
   ...
   *
   * This function is used to enable the digital rtc 8M clock to support
   * peripherals. For enabling the analog 8M clock, using `rtc_clk_8M_enable`
   * function above.
   */
   ...
   *
   * This function is used to disable the digital rtc 8M clock, which is only
   * used to support peripherals.
   */
   ...
   *
   * @param cal_val Average slow clock period in microseconds, fixed point value
   * as returned from `rtc_clk_cal`
  ...
  typedef struct
  {
    ...
    uint32_t
      cpu_fpu : 1; //!< Set to 1 to power down digital memories and CPU in sleep
...
#define RTC_SLEEP_PD_CONFIG_ALL(val)                                           \
  {                                                                            \
    .dig_fpu = (val), .rtc_fpu = (val), .cpu_fpu = (val), .i2s_fpu = (val),    \
    .bb_fpu = (val), .nrx_fpu = (val), .fe_fpu = (val),                        \
  }
...
#define RTC_SLEEP_CONFIG_DEFAULT(sleep_flags)                                  \
  { .lslp_mem_inf_fpu = 0,                                                     \
    ...
    .xtal_fpu =                                                                \
      is_dslp(sleep_flags) ? 0 : ((sleep_flags)&RTC_SLEEP_PD_XTAL) ? 0 : 1,    \
    .deep_slp_reject = 1,                                                      \
    .light_slp_reject = 1 };
...
#define RTC_SLEEP_PD_RTC_MEM_FOLLOW_CPU                                        \
  BIT(4) //!< RTC FAST and SLOW memories are automatically powered up and down
         //!< along with the CPU
...
#define RTC_SLEEP_PD_XTAL BIT(8)   //!< Power down main XTAL
   ...
   * entry into light sleep or deep sleep mode, switches APB and CPU clock
   * source (usually to XTAL), and sets bias voltages for digital and RTC power
   * domains.
   *
  ...
  /**
   * @brief Low level initialize for rtc state machine waiting cycles after
   * waking up
   *
   * This function configures the cycles chip need to wait for internal 8MHz
   * oscillator and external 40MHz crystal. As we configure fixed time for
   * waiting crystal, we need to pass period to calculate cycles. Now this
   * function only used in lightsleep mode.
   *
...
#define RTC_USB_TRIG_EN BIT(14)
   ...
   *
   * This function enters the sleep mode previously configured using
   * rtc_sleep_init function. Before entering sleep, software should configure
   * wake up sources appropriately (set up GPIO wakeup registers, timer wakeup
   * registers, and so on).
   *
   * If deep sleep mode was configured using rtc_sleep_init, and sleep is not
   * rejected by hardware (based on reject_opt flags), this function never
   * returns. When the chip wakes up from deep sleep, CPU is reset and execution
   * starts from ROM bootloader.
   *
   ...
   */
  uint32_t rtc_sleep_start(uint32_t wakeup_opt,
                           uint32_t reject_opt,
   ...
   *
   * Similar to rtc_sleep_start(), but additionally uses hardware to calculate
   * the CRC value of RTC FAST memory. On wake, this CRC is used to determine if
   * a deep sleep wake stub is valid to execute (if a wake address is set).
   *
   * No RAM is accessed while calculating the CRC and going into deep sleep,
   * which makes this function safe to use even if the caller's stack is in RTC
   * FAST memory.
   *
   * @note If no deep sleep wake stub address is set then calling
   * rtc_sleep_start() will have the same effect and takes less time as CRC
   * calculation is skipped.
   *
   * @note This function should only be called after rtc_sleep_init() has been
   * called to configure the system for deep sleep.
   *
  ...
  typedef struct
  {
    uint32_t ck8m_wait : 8; //!< Number of rtc_fast_clk cycles to wait for 8M
                            //!< clock to be ready
    uint32_t xtal_wait : 8; //!< Number of rtc_fast_clk cycles to wait for XTAL
                            //!< clock to be ready
    ...
    uint32_t
      cali_ocode : 1; //!< Calibrate Ocode to make bangap voltage more precise.
  } rtc_config_t;
...
#define RTC_CONFIG_DEFAULT()                                                   \
  {                                                                            \
    ...
    .pll_wait = RTC_CNTL_PLL_BUF_WAIT_DEFAULT, .clkctl_init = 1,               \
    .pwrctl_init = 1, .rtc_dboost_fpd = 1, .xtal_fpu = 0, .bbpll_fpu = 0,      \
    .cpu_waiti_clk_gate = 1, .cali_ocode = 0                                   \
  }
  ...
  typedef struct
  {
    uint32_t force : 1; //!< If 1, use configuration from RTC registers; if 0,
                        //!< use EFUSE/bootstrapping pins.
   ...
   * If there is no hardware calibration in process, start hardware calibration
   * and wait for calibration finished
   * @param cal_clk clock to be measured
   * @param slowclk_cycles if no hardware calibration in process, use this
   * amount of slow cycles to calibrate slowclk.
   */
