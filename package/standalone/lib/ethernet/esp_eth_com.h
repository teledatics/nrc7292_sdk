...
/**
 * @brief Ethernet frame header size: Dest addr(6 Bytes) + Src addr(6 Bytes) +
 * length/type(2 Bytes)
 *
 */
...
#define ETH_MAX_PACKET_SIZE                                                    \
  (ETH_HEADER_LEN + ETH_VLAN_TAG_LEN + ETH_MAX_PAYLOAD_LEN + ETH_CRC_LEN)
...
#define ETH_MIN_PACKET_SIZE (ETH_HEADER_LEN + ETH_MIN_PAYLOAD_LEN + ETH_CRC_LEN)
  ...
  struct esp_eth_mediator_s
  {
     ...
     *
     */
    esp_err_t (*phy_reg_read)(esp_eth_mediator_t* eth,
                              uint32_t phy_addr,
                              uint32_t phy_reg,
                              uint32_t* reg_value);
     ...
     */
    esp_err_t (*phy_reg_write)(esp_eth_mediator_t* eth,
                               uint32_t phy_addr,
                               uint32_t phy_reg,
                               uint32_t reg_value);
     ...
     *
     */
    esp_err_t (*stack_input)(esp_eth_mediator_t* eth,
                             uint8_t* buffer,
                             uint32_t length);
     ...
     *       - ESP_FAIL: process the new state failed because some error
     * occurred
     *
     */
    esp_err_t (*on_state_changed)(esp_eth_mediator_t* eth,
                                  esp_eth_state_t state,
                                  void* args);
  };
   ...
   */
  esp_err_t esp_eth_detect_phy_addr(esp_eth_mediator_t* eth,
                                    int* detected_addr);
