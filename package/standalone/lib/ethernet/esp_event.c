...
#ifdef CONFIG_ESP_EVENT_LOOP_PROFILING
static SLIST_HEAD(esp_event_loop_instance_list_t, esp_event_loop_instance)
  s_event_loops = SLIST_HEAD_INITIALIZER(s_event_loops);
...
esp_event_dump_prepare(void)
{
  ...
  // Count the number of items to be printed. This is needed to compute how much
  // memory to reserve.
  ...
  SLIST_FOREACH(loop_it, &s_event_loops, next)
  {
    ...
    {
      SLIST_FOREACH(handler_it, &(loop_node_it->handlers), next) { handlers++; }
  ...
  int allowance = 3;
  int size =
    (((loops + allowance) * (sizeof(LOOP_DUMP_FORMAT) + 10 + 20 + 2 * 11)) +
     ((handlers + allowance) *
      (sizeof(HANDLER_DUMP_FORMAT) + 10 + 2 * 20 + 11 + 20)));
...
static void
esp_event_loop_run_task(void* args)
{
  esp_err_t err;
...
static void
handler_execute(esp_event_loop_instance_t* loop,
                esp_event_handler_node_t* handler,
                esp_event_post_instance_t post)
{
  ESP_LOGD(TAG,
           "running post %s:%d with handler %p and context %p on loop %p",
           post.base,
           (int)post.id,
           handler->handler_ctx->handler,
           &handler->handler_ctx,
           loop);
  ...
  (*(handler->handler_ctx->handler))(
    handler->handler_ctx->arg, post.base, post.id, data_ptr);
#else
  (*(handler->handler_ctx->handler))(
    handler->handler_ctx->arg, post.base, post.id, post.data);
#endif
...
static esp_err_t
handler_instances_add(esp_event_handler_nodes_t* handlers,
                      esp_event_handler_t event_handler,
                      void* event_handler_arg,
                      esp_event_handler_instance_context_t** handler_ctx,
                      bool legacy)
{
  esp_event_handler_node_t* handler_instance =
    calloc(1, sizeof(*handler_instance));
...
static esp_err_t
base_node_add_handler(esp_event_base_node_t* base_node,
                      int32_t id,
                      esp_event_handler_t event_handler,
                      void* event_handler_arg,
                      esp_event_handler_instance_context_t** handler_ctx,
                      bool legacy)
{
  if (id == ESP_EVENT_ANY_ID) {
    return handler_instances_add(&(base_node->handlers),
                                 event_handler,
                                 event_handler_arg,
                                 handler_ctx,
                                 legacy);
    ...
    if (!last_id_node || !id_node) {
      ...
      err = handler_instances_add(&(id_node->handlers),
                                  event_handler,
                                  event_handler_arg,
                                  handler_ctx,
                                  legacy);
      ...
      return handler_instances_add(&(id_node->handlers),
                                   event_handler,
                                   event_handler_arg,
                                   handler_ctx,
                                   legacy);
...
static esp_err_t
loop_node_add_handler(esp_event_loop_node_t* loop_node,
                      ...
                      bool legacy)
{
  if (base == esp_event_any_base && id == ESP_EVENT_ANY_ID) {
    return handler_instances_add(&(loop_node->handlers),
                                 event_handler,
                                 event_handler_arg,
                                 handler_ctx,
                                 legacy);
    ...
    if (!last_base_node || !base_node ||
        (base_node && !SLIST_EMPTY(&(base_node->id_nodes)) &&
         id == ESP_EVENT_ANY_ID) ||
        (last_base_node && last_base_node->base != base &&
         !SLIST_EMPTY(&(last_base_node->id_nodes)) && id == ESP_EVENT_ANY_ID)) {
      ...
      err = base_node_add_handler(
        base_node, id, event_handler, event_handler_arg, handler_ctx, legacy);
...
static esp_err_t
handler_instances_remove(esp_event_handler_nodes_t* handlers,
                         esp_event_handler_instance_context_t* handler_ctx,
                         bool legacy)
{
...
static esp_err_t
base_node_remove_handler(esp_event_base_node_t* base_node,
                         int32_t id,
                         esp_event_handler_instance_context_t* handler_ctx,
                         bool legacy)
{
  if (id == ESP_EVENT_ANY_ID) {
    return handler_instances_remove(
      &(base_node->handlers), handler_ctx, legacy);
  } else {
    ...
    SLIST_FOREACH_SAFE(it, &(base_node->id_nodes), next, temp)
    {
      if (it->id == id) {
        esp_err_t res =
          handler_instances_remove(&(it->handlers), handler_ctx, legacy);
...
static esp_err_t
loop_node_remove_handler(esp_event_loop_node_t* loop_node,
                         esp_event_base_t base,
                         int32_t id,
                         esp_event_handler_instance_context_t* handler_ctx,
                         bool legacy)
{
  if (base == esp_event_any_base && id == ESP_EVENT_ANY_ID) {
    return handler_instances_remove(
      &(loop_node->handlers), handler_ctx, legacy);
  } else {
      ...
      if (it->base == base) {
        ...
        if (res == ESP_OK) {
          if (SLIST_EMPTY(&(it->handlers)) && SLIST_EMPTY(&(it->id_nodes))) {
            SLIST_REMOVE(
              &(loop_node->base_nodes), it, esp_event_base_node, next);
            free(it);
            return ESP_OK;
          }
        }
...
static void
handler_instances_remove_all(esp_event_handler_nodes_t* handlers)
{
  esp_event_handler_node_t *it, *temp;
  SLIST_FOREACH_SAFE(it, handlers, next, temp)
  {
...
static void
base_node_remove_all_handler(esp_event_base_node_t* base_node)
{
...
static void
loop_node_remove_all_handler(esp_event_loop_node_t* loop_node)
{
...
post_instance_delete(esp_event_post_instance_t* post)
{
#if CONFIG_ESP_EVENT_POST_FROM_ISR
...
esp_err_t
esp_event_loop_create(const esp_event_loop_args_t* event_loop_args,
                      esp_event_loop_handle_t* event_loop)
{
  ...
  loop->queue = xQueueCreate(event_loop_args->queue_size,
  ...
  if (event_loop_args->task_name != NULL) {
    ...
    //         xTaskCreatePinnedToCore(esp_event_loop_run_task,
    ...
    BaseType_t task_created = xTaskCreate(esp_event_loop_run_task,
                                          event_loop_args->task_name,
...
// On event lookup performance: The library implements the event list as a
// linked list, which results to O(n) lookup time. The test comparing this
// implementation to the O(lg n) performance of rbtrees
// (https://github.com/freebsd/freebsd/blob/master/sys/sys/tree.h)
// indicate that the difference is not that substantial, especially considering
// the additional pointers per node of rbtrees. Code for the rbtree
// implementation of the event loop library is archived in
// feature/esp_event_loop_library_rbtrees if needed.
esp_err_t
esp_event_loop_run(esp_event_loop_handle_t event_loop, TickType_t ticks_to_run)
{
  assert(event_loop);
  ...
  while (xQueueReceive(loop->queue, &post, ticks_to_run) == pdTRUE) {
    ...
    SLIST_FOREACH_SAFE(loop_node, &(loop->loop_nodes), next, temp_node)
    {
      ...
      SLIST_FOREACH_SAFE(base_node, &(loop_node->base_nodes), next, temp_base)
      {
        if (base_node->base == post.base) {
          ...
          SLIST_FOREACH_SAFE(
            handler, &(base_node->handlers), next, temp_handler)
          {
            handler_execute(loop, handler, post);
            exec |= true;
          }
          ...
          SLIST_FOREACH_SAFE(
            id_node, &(base_node->id_nodes), next, temp_id_node)
          {
            if (id_node->id == post.id) {
              // Execute id level handlers
              SLIST_FOREACH_SAFE(
                handler, &(id_node->handlers), next, temp_handler)
              {
                handler_execute(loop, handler, post);
                exec |= true;
              }
    ...
    if (!exec) {
      ...
      ESP_LOGD(
        TAG,
        "no handlers have been registered for event %s:%d posted to loop %p",
        base,
        (int)id,
        event_loop);
    }
...
esp_err_t
esp_event_loop_delete(esp_event_loop_handle_t event_loop)
{
  assert(event_loop);
  ...
  bool legacy)
{
  ...
  bool is_loop_level_handler =
    (event_base == esp_event_any_base) && (event_id == ESP_EVENT_ANY_ID);

  if (!last_loop_node ||
      (last_loop_node && !SLIST_EMPTY(&(last_loop_node->base_nodes)) &&
    ...
    err = loop_node_add_handler(loop_node,
                                event_base,
                                event_id,
                                event_handler,
                                event_handler_arg,
    ...
    err = loop_node_add_handler(last_loop_node,
                                event_base,
                                event_id,
                                event_handler,
                                event_handler_arg,
...
esp_event_handler_register_with(esp_event_loop_handle_t event_loop,
                                ...
                                void* event_handler_arg)
{
  return esp_event_handler_register_with_internal(event_loop,
                                                  event_base,
                                                  event_id,
                                                  event_handler,
                                                  event_handler_arg,
...
{
  return esp_event_handler_register_with_internal(
    event_loop,
    event_base,
    event_id,
    event_handler,
  ...
  bool legacy)
{
  ...
  if (event_base == ESP_EVENT_ANY_BASE && event_id != ESP_EVENT_ANY_ID) {
    ESP_LOGE(TAG,
             "unregistering to any event base with specific id unsupported");
    return ESP_FAIL;
  }
  ...
  {
    esp_err_t res =
      loop_node_remove_handler(it, event_base, event_id, handler_ctx, legacy);

    if (res == ESP_OK && SLIST_EMPTY(&(it->base_nodes)) &&
        SLIST_EMPTY(&(it->handlers))) {
      SLIST_REMOVE(&(loop->loop_nodes), it, esp_event_loop_node, next);
      free(it);
      break;
    }
                                  ...
                                  esp_event_handler_t event_handler)
{
  ...
  return esp_event_handler_unregister_with_internal(
    event_loop, event_base, event_id, &local_handler_ctx, true);
}
...
esp_err_t
esp_event_post_to(esp_event_loop_handle_t event_loop,
                  esp_event_base_t event_base,
                  int32_t event_id,
                  void* event_data,
                  ...
                  TickType_t ticks_to_wait)
{
  ...
  // Find the task that currently executes the loop. It is safe to query
  // loop->task since it is not mutated since loop creation. ENSURE THIS REMAINS
  // TRUE.
  if (loop->task == NULL) {
    // The loop has no dedicated task. Find out what task is currently running
    // it.
...
#if CONFIG_ESP_EVENT_POST_FROM_ISR
esp_err_t
esp_event_isr_post_to(esp_event_loop_handle_t event_loop,
                      esp_event_base_t event_base,
                      int32_t event_id,
                      void* event_data,
                      size_t event_data_size,
...
esp_event_dump(FILE* file)
{
  ...
  SLIST_FOREACH(loop_it, &s_event_loops, next)
  {
    ...
    PRINT_DUMP_INFO(dst,
                    ...
                    LOOP_DUMP_FORMAT,
                    loop_it,
                    loop_it->task != NULL ? loop_it->name : "none",
                    events_recieved,
                    events_dropped);
    ...
    SLIST_FOREACH(loop_node_it, &(loop_it->loop_nodes), next)
    {
      SLIST_FOREACH(handler_it, &(loop_node_it->handlers), next)
      {
        PRINT_DUMP_INFO(dst,
                        sz,
                        HANDLER_DUMP_FORMAT,
                        handler_it->handler_ctx->handler,
                        "ESP_EVENT_ANY_BASE",
                        "ESP_EVENT_ANY_ID",
                        handler_it->invoked,
                        handler_it->time);
