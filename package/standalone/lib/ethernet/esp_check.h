...
/**
 * Macro which can be used to check the error code. If the code is not ESP_OK,
 * it prints the message and returns.
 */
...
#define ESP_RETURN_ON_ERROR(x, log_tag, format, ...)                           \
  do {                                                                         \
    esp_err_t err_rc_ = (x);                                                   \
    if (unlikely(err_rc_ != ESP_OK)) {                                         \
      nrc_usr_print("%s(%d): " format, __FUNCTION__, __LINE__, ##__VA_ARGS__); \
      return err_rc_;                                                          \
    }                                                                          \
  } while (0)
#endif
...
/**
 * Macro which can be used to check the error code. If the code is not ESP_OK,
 * it prints the message, sets the local variable 'ret' to the code, and then
 * exits by jumping to 'goto_tag'.
 */
#if defined(CONFIG_COMPILER_OPTIMIZATION_CHECKS_SILENT)
...
/**
 * Macro which can be used to check the condition. If the condition is not
 * 'true', it prints the message and returns with the supplied 'err_code'.
 */
#if defined(CONFIG_COMPILER_OPTIMIZATION_CHECKS_SILENT)
...
/**
 * Macro which can be used to check the condition. If the condition is not
 * 'true', it prints the message, sets the local variable 'ret' to the supplied
 * 'err_code', and then exits by jumping to 'goto_tag'.
 */
#if defined(CONFIG_COMPILER_OPTIMIZATION_CHECKS_SILENT)
...
#define ESP_GOTO_ON_FALSE_ISR(a, err_code, goto_tag, log_tag, format, ...)     \
  do {                                                                         \
    if (unlikely(!(a))) {                                                      \
      ESP_EARLY_LOGE(                                                          \
        log_tag, "%s(%d): " format, __FUNCTION__, __LINE__, ##__VA_ARGS__);    \
      ret = err_code;                                                          \
      goto goto_tag;                                                           \
    }                                                                          \
  } while (0)
