...
#define cpu_hal_waiti() cpu_ll_waiti()
  ...
  /**
   * Set and enable a watchpoint, specifying the memory range and trigger
   * operation.
   *
   * @param id watchpoint to set [0..SOC_CPU_WATCHPOINTS_NUM - 1]
   * @param addr starting address
   * @param size number of bytes from starting address to watch
   * @param trigger operation on specified memory range that triggers the
   * watchpoint (read, write, read/write)
   */
  void cpu_hal_set_watchpoint(int id,
                              const void* addr,
                              size_t size,
