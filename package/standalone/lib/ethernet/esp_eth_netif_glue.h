...
extern "C"
{
  ...
  /**
   * @brief Handle of netif glue - an intermediate layer between netif and
   * Ethernet driver
   *
   */
   ...
   *
   * @note: Ethernet handle might not yet properly initialized when setting up
   * these default handlers
   * @warning: This function is deprecated and is kept here only for
   * compatibility reasons. Registration of default IP layer handlers for
   * Ethernet is now handled automatically. Do not call this function if you
   * want to use multiple Ethernet instances at a time.
   *
   * @param[in] esp_netif esp network interface handle created for Ethernet
   * driver
   * @return
   ...
   *
   * @warning: This function is deprecated and is kept here only for
   * compatibility reasons. Unregistration of default IP layer handlers for
   * Ethernet is now handled automatically if not registered by calling
