...
#define ESP_IP6_ADDR_BLOCK1(ip6addr)                                           \
  ((uint16_t)((esp_netif_htonl((ip6addr)->addr[0]) >> 16) & 0xffff))
#define ESP_IP6_ADDR_BLOCK2(ip6addr)                                           \
  ((uint16_t)((esp_netif_htonl((ip6addr)->addr[0])) & 0xffff))
#define ESP_IP6_ADDR_BLOCK3(ip6addr)                                           \
  ((uint16_t)((esp_netif_htonl((ip6addr)->addr[1]) >> 16) & 0xffff))
#define ESP_IP6_ADDR_BLOCK4(ip6addr)                                           \
  ((uint16_t)((esp_netif_htonl((ip6addr)->addr[1])) & 0xffff))
#define ESP_IP6_ADDR_BLOCK5(ip6addr)                                           \
  ((uint16_t)((esp_netif_htonl((ip6addr)->addr[2]) >> 16) & 0xffff))
#define ESP_IP6_ADDR_BLOCK6(ip6addr)                                           \
  ((uint16_t)((esp_netif_htonl((ip6addr)->addr[2])) & 0xffff))
#define ESP_IP6_ADDR_BLOCK7(ip6addr)                                           \
  ((uint16_t)((esp_netif_htonl((ip6addr)->addr[3]) >> 16) & 0xffff))
#define ESP_IP6_ADDR_BLOCK8(ip6addr)                                           \
  ((uint16_t)((esp_netif_htonl((ip6addr)->addr[3])) & 0xffff))

#define IPSTR "%d.%d.%d.%d"
#define esp_ip4_addr_get_byte(ipaddr, idx)                                     \
...
#define IPV62STR(ipaddr)                                                       \
  ESP_IP6_ADDR_BLOCK1(&(ipaddr)), ESP_IP6_ADDR_BLOCK2(&(ipaddr)),              \
    ESP_IP6_ADDR_BLOCK3(&(ipaddr)), ESP_IP6_ADDR_BLOCK4(&(ipaddr)),            \
    ESP_IP6_ADDR_BLOCK5(&(ipaddr)), ESP_IP6_ADDR_BLOCK6(&(ipaddr)),            \
    ESP_IP6_ADDR_BLOCK7(&(ipaddr)), ESP_IP6_ADDR_BLOCK8(&(ipaddr))
...
#define ESP_IP6ADDR_INIT(a, b, c, d)                                           \
  { .type = ESP_IPADDR_TYPE_V6,                                                \
    .u_addr = { .ip6 = { .addr = { a, b, c, d }, .zone = 0 } } };
   ...
   */
  static inline void esp_netif_ip_addr_copy(esp_ip_addr_t* dest,
                                            const esp_ip_addr_t* src)
  {
    dest->type = src->type;
