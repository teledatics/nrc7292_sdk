...
#define esp_rom_delay_us(x) _delay_us(x)
...
static esp_err_t
esp_emac_alloc_driver_obj(const eth_mac_config_t* config,
                          emac_esp32_t** emac_out_hdl,
                          void** out_descriptors);
static void
esp_emac_free_driver_obj(emac_esp32_t* emac, void* descriptors);
static esp_err_t
emac_esp32_start(esp_eth_mac_t* mac);
static esp_err_t
emac_esp32_stop(esp_eth_mac_t* mac);

static esp_err_t
emac_esp32_set_mediator(esp_eth_mac_t* mac, esp_eth_mediator_t* eth)
{
  esp_err_t ret = ESP_OK;
  ESP_GOTO_ON_FALSE(
    eth, ESP_ERR_INVALID_ARG, err, TAG, "can't set mac's mediator to null");
...
static esp_err_t
emac_esp32_write_phy_reg(esp_eth_mac_t* mac,
                         uint32_t phy_addr,
                         uint32_t phy_reg,
                         uint32_t reg_value)
{
  esp_err_t ret = ESP_OK;
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
  ESP_GOTO_ON_FALSE(!emac_ll_is_mii_busy(emac->hal.mac_regs),
...
static esp_err_t
emac_esp32_read_phy_reg(esp_eth_mac_t* mac,
                        uint32_t phy_addr,
                        uint32_t phy_reg,
                        uint32_t* reg_value)
{
  esp_err_t ret = ESP_OK;
  ESP_GOTO_ON_FALSE(
    reg_value, ESP_ERR_INVALID_ARG, err, TAG, "can't set reg_value to null");
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
  ESP_GOTO_ON_FALSE(!emac_ll_is_mii_busy(emac->hal.mac_regs),
                    ESP_ERR_INVALID_STATE,
                    err,
...
static esp_err_t
emac_esp32_set_addr(esp_eth_mac_t* mac, uint8_t* addr)
{
  esp_err_t ret = ESP_OK;
  ESP_GOTO_ON_FALSE(
    addr, ESP_ERR_INVALID_ARG, err, TAG, "can't set mac addr to null");
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
...
static esp_err_t
emac_esp32_get_addr(esp_eth_mac_t* mac, uint8_t* addr)
{
  esp_err_t ret = ESP_OK;
  ESP_GOTO_ON_FALSE(
    addr, ESP_ERR_INVALID_ARG, err, TAG, "can't set mac addr to null");
...
static esp_err_t
emac_esp32_set_link(esp_eth_mac_t* mac, eth_link_t link)
{
  esp_err_t ret = ESP_OK;
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
  switch (link) {
    case ETH_LINK_UP:
      ESP_GOTO_ON_ERROR(
        esp_intr_enable(emac->intr_hdl), err, TAG, "enable interrupt failed");
      emac_esp32_start(mac);
      break;
    case ETH_LINK_DOWN:
      ESP_GOTO_ON_ERROR(
        esp_intr_disable(emac->intr_hdl), err, TAG, "disable interrupt failed");
      emac_esp32_stop(mac);
      break;
    default:
      ESP_GOTO_ON_FALSE(
        false, ESP_ERR_INVALID_ARG, err, TAG, "unknown link status");
      break;
  }
  return ESP_OK;
err:
...
static esp_err_t
emac_esp32_set_speed(esp_eth_mac_t* mac, eth_speed_t speed)
{
...
static esp_err_t
emac_esp32_set_duplex(esp_eth_mac_t* mac, eth_duplex_t duplex)
{
  esp_err_t ret = ESP_ERR_INVALID_ARG;
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
  if (duplex == ETH_DUPLEX_HALF || duplex == ETH_DUPLEX_FULL) {
    ...
    ESP_LOGD(
      TAG, "working in %s duplex", duplex == ETH_DUPLEX_HALF ? "half" : "full");
    return ESP_OK;
  }
  return ret;
}
...
static esp_err_t
emac_esp32_set_promiscuous(esp_eth_mac_t* mac, bool enable)
{
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
...
static esp_err_t
emac_esp32_enable_flow_ctrl(esp_eth_mac_t* mac, bool enable)
{
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
...
static esp_err_t
emac_esp32_set_peer_pause_ability(esp_eth_mac_t* mac, uint32_t ability)
{
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
...
static esp_err_t
emac_esp32_transmit(esp_eth_mac_t* mac, uint8_t* buf, uint32_t length)
{
  esp_err_t ret = ESP_OK;
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
  uint32_t sent_len = emac_hal_transmit_frame(&emac->hal, buf, length);
  ESP_GOTO_ON_FALSE(sent_len == length,
...
static esp_err_t
emac_esp32_receive(esp_eth_mac_t* mac, uint8_t* buf, uint32_t* length)
{
  esp_err_t ret = ESP_OK;
  ...
  uint32_t receive_len = emac_hal_receive_frame(&emac->hal,
                                                buf,
                                                expected_len,
                                                &emac->frames_remain,
                                                &emac->free_rx_descriptor);
  /* we need to check the return value in case the buffer size is not enough */
  //     ESP_LOGD(TAG, "receive len= %d", receive_len);
  ESP_GOTO_ON_FALSE(expected_len >= receive_len,
                    ESP_ERR_INVALID_SIZE,
                    err,
                    TAG,
...
emac_esp32_rx_task(void* arg)
{
  ...
  while (1) {
    ...
    do {
      ...
      // we need to do extra checking of remained frames in case there are no
      // unhandled frames left, but pause frame is still undergoing
      if ((emac->free_rx_descriptor < emac->flow_control_low_water_mark) &&
          emac->do_flow_ctrl && emac->frames_remain) {
        emac_ll_pause_frame_enable(emac->hal.ext_regs, true);
      } else if ((emac->free_rx_descriptor >
                  emac->flow_control_high_water_mark) ||
                 !emac->frames_remain) {
...
static void
emac_esp32_init_smi_gpio(emac_esp32_t* emac)
{
  //     if (emac->smi_mdc_gpio_num >= 0) {
  /* Setup SMI MDC GPIO */
  //         gpio_set_direction(emac->smi_mdc_gpio_num, GPIO_MODE_OUTPUT);
  //         esp_rom_gpio_connect_out_signal(emac->smi_mdc_gpio_num,
  //         EMAC_MDC_O_IDX, false, false);
  //         gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[emac->smi_mdc_gpio_num],
  //         PIN_FUNC_GPIO);
  //     }
  //     if (emac->smi_mdio_gpio_num >= 0) {
  /* Setup SMI MDIO GPIO */
  //         gpio_set_direction(emac->smi_mdio_gpio_num,
  //         GPIO_MODE_INPUT_OUTPUT);
  //         esp_rom_gpio_connect_out_signal(emac->smi_mdio_gpio_num,
  //         EMAC_MDO_O_IDX, false, false);
  //         esp_rom_gpio_connect_in_signal(emac->smi_mdio_gpio_num,
  //         EMAC_MDI_I_IDX, false);
  //         gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[emac->smi_mdio_gpio_num],
  //         PIN_FUNC_GPIO);
  //     }
}

static esp_err_t
emac_config_apll_clock(void)
{
  //     uint32_t expt_freq = 50000000; // 50MHz
  //     uint32_t real_freq = 0;
  //     esp_err_t ret = periph_rtc_apll_freq_set(expt_freq, &real_freq);
  //     ESP_RETURN_ON_FALSE(ret != ESP_ERR_INVALID_ARG, ESP_FAIL, TAG, "Set
  //     APLL clock coefficients failed"); if (ret == ESP_ERR_INVALID_STATE) {
  //         ESP_LOGW(TAG, "APLL is occupied already, it is working at %d Hz",
  //         real_freq);
  //     }
  //     // If the difference of real APLL frequency is not within 50 ppm, i.e.
  //     2500 Hz, the APLL is unavailable ESP_RETURN_ON_FALSE(abs((int)real_freq
  //     - (int)expt_freq) <= 2500,
  //                          ESP_ERR_INVALID_STATE, TAG, "The APLL is working
  //                          at an unusable frequency");

  return ESP_OK;
}

static esp_err_t
emac_esp32_init(esp_eth_mac_t* mac)
{
  //     esp_err_t ret = ESP_OK;
  //     emac_esp32_t *emac = __containerof(mac, emac_esp32_t, parent);
  ...
  //     ESP_GOTO_ON_ERROR(eth->on_state_changed(eth, ETH_STATE_LLINIT, NULL),
  //     err, TAG, "lowlevel init failed");
  //     /* software reset */
  ...
  //     }
  //     ESP_GOTO_ON_FALSE(to < emac->sw_reset_timeout_ms / 10, ESP_ERR_TIMEOUT,
  //     err, TAG, "reset timeout");
  //     /* set smi clock */
  ...
  //     emac_hal_dma_config_t dma_config = { .dma_burst_len =
  //     emac->dma_burst_len }; emac_hal_init_dma_default(&emac->hal,
  //     &dma_config);
  //     /* get emac address from efuse */
  //     ESP_GOTO_ON_ERROR(esp_read_mac(emac->addr, ESP_MAC_ETH), err, TAG,
...
static esp_err_t
emac_esp32_deinit(esp_eth_mac_t* mac)
{
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
  esp_eth_mediator_t* eth = emac->eth;
#ifdef CONFIG_PM_ENABLE
...
static esp_err_t
emac_esp32_del(esp_eth_mac_t* mac)
{
  emac_esp32_t* emac = __containerof(mac, emac_esp32_t, parent);
...
IRAM_ATTR void
emac_isr_default_handler(void* args)
{
                          ...
                          void** out_descriptors)
{
  ...
  if (config->flags & ETH_MAC_FLAG_WORK_WITH_CACHE_DISABLE) {
    emac = heap_caps_calloc(
      1, sizeof(emac_esp32_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
  } else {
    emac = calloc(1, sizeof(emac_esp32_t));
  }
  ESP_GOTO_ON_FALSE(
    emac, ESP_ERR_NO_MEM, err, TAG, "no mem for esp emac object");
  ...
  ESP_GOTO_ON_FALSE(
    descriptors, ESP_ERR_NO_MEM, err, TAG, "no mem for descriptors");
  /* alloc memory for ethernet dma buffer */
  for (int i = 0; i < CONFIG_ETH_DMA_RX_BUFFER_NUM; i++) {
    emac->rx_buf[i] =
      heap_caps_calloc(1, CONFIG_ETH_DMA_BUFFER_SIZE, MALLOC_CAP_DMA);
    ESP_GOTO_ON_FALSE(
      emac->rx_buf[i], ESP_ERR_NO_MEM, err, TAG, "no mem for RX DMA buffers");
  }
  for (int i = 0; i < CONFIG_ETH_DMA_TX_BUFFER_NUM; i++) {
    emac->tx_buf[i] =
      heap_caps_calloc(1, CONFIG_ETH_DMA_BUFFER_SIZE, MALLOC_CAP_DMA);
    ESP_GOTO_ON_FALSE(
      emac->tx_buf[i], ESP_ERR_NO_MEM, err, TAG, "no mem for TX DMA buffers");
  }
  /* alloc PM lock */
#ifdef CONFIG_PM_ENABLE
  ESP_GOTO_ON_ERROR(
    esp_pm_lock_create(ESP_PM_APB_FREQ_MAX, 0, "emac_esp32", &emac->pm_lock),
    err,
    TAG,
  ...
  //     BaseType_t xReturned = xTaskCreatePinnedToCore(emac_esp32_rx_task,
  //     "emac_rx", config->rx_task_stack_size, emac,
  //                            config->rx_task_prio, &emac->rx_task_hdl,
  //                            core_num);
  BaseType_t xReturned = xTaskCreate(emac_esp32_rx_task,
                                     "emac_rx",
                                     ...
                                     &emac->rx_task_hdl);
  ESP_GOTO_ON_FALSE(
    xReturned == pdPASS, ESP_FAIL, err, TAG, "create emac_rx task failed");
...
typedef char eth_esp32_emac_config_t;

static esp_err_t
esp_emac_config_data_interface(const eth_esp32_emac_config_t* esp32_emac_config,
                               emac_esp32_t* emac)
{
  esp_err_t ret = ESP_OK;
  ...
  //         // by default, the clock mode is selected at compile time (by
  //         Kconfig) if (esp32_emac_config->clock_config.rmii.clock_mode ==
  //         EMAC_CLK_DEFAULT) {
  // #if CONFIG_ETH_RMII_CLK_INPUT
  // #if CONFIG_ETH_RMII_CLK_IN_GPIO == 0
  //             emac->clock_config.rmii.clock_mode = EMAC_CLK_EXT_IN;
  //             emac->clock_config.rmii.clock_gpio =
  ...
  //             ESP_GOTO_ON_FALSE(emac->clock_config.rmii.clock_gpio ==
  //             EMAC_CLK_IN_GPIO,
  //                               ESP_ERR_INVALID_ARG, err, TAG, "ESP32 EMAC
  //                               only support input RMII clock to GPIO0");
  ...
  //                               EMAC_CLK_OUT_GPIO ||
  //                               emac->clock_config.rmii.clock_gpio ==
  //                               EMAC_CLK_OUT_180_GPIO, ESP_ERR_INVALID_ARG,
  //                               err, TAG, "invalid EMAC clock output GPIO");
  //             emac_hal_iomux_rmii_clk_ouput(emac->clock_config.rmii.clock_gpio);
  //             if (emac->clock_config.rmii.clock_gpio ==
  //             EMAC_APPL_CLK_OUT_GPIO) {
  ...
  //             ESP_GOTO_ON_ERROR(emac_config_apll_clock(), err, TAG,
  //             "Configure APLL for RMII failed"); emac->use_apll = true;
  //         } else {
  ...
  //         break;
  //     default:
  //         ESP_GOTO_ON_FALSE(false, ESP_ERR_INVALID_ARG, err, TAG, "invalid
  //         EMAC Data Interface:%d", esp32_emac_config->interface);
  //     }
  // err:
  return ret;
}
...
esp_eth_mac_t*
esp_eth_mac_new_esp32(const eth_esp32_emac_config_t* esp32_config,
                      const eth_mac_config_t* config)
{
  ...
  ESP_GOTO_ON_FALSE(
    ret_code == ESP_OK, NULL, err, TAG, "alloc driver object failed");
  ...
  /* alloc interrupt */
  if (config->flags & ETH_MAC_FLAG_WORK_WITH_CACHE_DISABLE) {
    //         ret_code = esp_intr_alloc(ETS_ETH_MAC_INTR_SOURCE,
    //         ESP_INTR_FLAG_IRAM,
    //                                   emac_isr_default_handler, &emac->hal,
    //                                   &(emac->intr_hdl));
  } else {
  ...
  //     ESP_GOTO_ON_FALSE(ret_code == ESP_OK, NULL, err, TAG, "alloc emac
  //     interrupt failed");
  ret_code = esp_emac_config_data_interface(esp32_config, emac);
  //     ESP_GOTO_ON_FALSE(ret_code == ESP_OK, NULL, err_interf, TAG, "config
  //     emac interface failed");
