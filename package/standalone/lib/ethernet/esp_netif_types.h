...
#define ESP_ERR_ESP_NETIF_IP6_ADDR_FAILED ESP_ERR_ESP_NETIF_BASE + 0x0C
  ...
  typedef enum
  {
    ...
    ESP_NETIF_DNS_BACKUP, /**< DNS backup server address (Wi-Fi STA and Ethernet
                             only) */
    ESP_NETIF_DNS_FALLBACK, /**< DNS fallback server address (Wi-Fi STA and
                               Ethernet only) */
    ESP_NETIF_DNS_MAX
  ...
  {
    ESP_NETIF_DHCP_INIT =
      0, /**< DHCP client/server is in initial state (not yet started) */
  ...
  typedef enum
  {
    ...
    ESP_NETIF_VENDOR_CLASS_IDENTIFIER =
      60, /**< Vendor Class Identifier of a DHCP client */
    ESP_NETIF_VENDOR_SPECIFIC_INFO =
      43, /**< Vendor Specific Information of a DHCP server */
  ...
  typedef struct
  {
    int if_index; /*!< Interface index for which the event is received (left for
                     legacy compilation) */
  ...
  //      3) network stack specific config (esp_netif_net_stack_ifconfig_t) --
  ...
  typedef struct esp_netif_inherent_config
  {
    ...
    const esp_netif_ip_info_t*
      ip_info; /*!< initial ip address for this interface */
    uint32_t
      get_ip_event; /*!< event id to be raised when interface gets an IP */
    uint32_t
      lost_ip_event; /*!< event id to be raised when interface losts its IP */
  ...
  struct esp_netif_driver_ifconfig
  {
    ...
    esp_err_t (*transmit_wrap)(void* h,
                               void* buffer,
                               size_t len,
   ...
   */
  typedef esp_err_t (*esp_netif_receive_t)(esp_netif_t* esp_netif,
                                           void* buffer,
                                           size_t len,
                                           void* eb);
