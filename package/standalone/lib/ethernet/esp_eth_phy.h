...
#define ESP_ETH_PHY_ADDR_AUTO (-1)
  ...
  struct esp_eth_phy_s
  {
     ...
     *      - ESP_ERR_INVALID_ARG: set mediator for Ethernet PHY instance failed
     * because of some invalid arguments
     *
     */
     ...
     *      - ESP_FAIL: initialize Ethernet PHY failed because some error
     * occurred
     *
     */
     ...
     *      - ESP_FAIL: deinitialize Ethernet PHY failed because some error
     * occurred
     *
     */
     ...
     *
     * @param[in] phy: Ethernet PHY instance
     * @param[in] cmd: Configuration command, it is possible to Enable
     * (restart), Disable or get current status of PHY auto negotiation
     * @param[out] autonego_en_stat: Address where to store current status of
     * auto negotiation configuration
     *
     * @return
     *      - ESP_OK: restart auto negotiation successfully
     *      - ESP_FAIL: restart auto negotiation failed because some error
     * occurred
     *      - ESP_ERR_INVALID_ARG: invalid command
     *
     */
    esp_err_t (*autonego_ctrl)(esp_eth_phy_t* phy,
     ...
     *      - ESP_FAIL: get Ethernet PHY link status failed because some error
     * occurred
     *
     */
     ...
     *
     * @param[in] phy: Ethernet PHY instance
     * @param[in] enable: set true to power on Ethernet PHY; ser false to power
     * off Ethernet PHY
     *
     * @return
     *      - ESP_OK: control Ethernet PHY power successfully
     *      - ESP_FAIL: control Ethernet PHY power failed because some error
     * occurred
     *
     */
     ...
     *      - ESP_FAIL: set Ethernet PHY address failed because some error
     * occurred
     *
     */
     ...
     *      - ESP_ERR_INVALID_ARG: get Ethernet PHY address failed because of
     * invalid argument
     *
     */
     ...
     *      - ESP_ERR_INVALID_ARG: Advertise pause ability failed because of
     * invalid argument
     *
     */
     ...
     *
     * @return
     *      - ESP_OK: PHY instance loopback mode has been configured
     * successfully
     *      - ESP_FAIL: PHY instance loopback configuration failed because some
     * error occurred
     *
     */
     ...
     *
     * @note Autonegotiation feature needs to be disabled prior to calling this
     * function for the new setting to be applied
     *
     * @param[in] phy: Ethernet PHY instance
     ...
     *      - ESP_FAIL: PHY instance speed mode configuration failed because
     * some error occurred
     *
     */
     ...
     *      - ESP_FAIL: PHY instance duplex mode configuration failed because
     * some error occurred
     *
     */
  ...
  typedef struct
  {
    int32_t phy_addr; /*!< PHY address, set -1 to enable PHY address detection
                         at initialization stage */
    uint32_t reset_timeout_ms; /*!< Reset timeout value (Unit: ms) */
    uint32_t
      autonego_timeout_ms; /*!< Auto-negotiation timeout value (Unit: ms) */
...
#define ETH_PHY_DEFAULT_CONFIG()                                               \
   ...
   *
   * @note For ESP-IDF backwards compatibility reasons. In all other cases, use
   * esp_eth_phy_new_lan87xx instead.
   *
   * @param[in] config: configuration of PHY
   *
   * @return
   ...
   *      - NULL: create PHY instance failed because some error occurred
   */
  static inline esp_eth_phy_t* esp_eth_phy_new_ksz8091(
    const eth_phy_config_t* config)
  {
