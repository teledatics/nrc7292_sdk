...
extern "C"
{
  ...
  struct esp_eth_mac_s
  {
     ...
     *      - ESP_ERR_INVALID_ARG: set mediator for Ethernet MAC failed because
     * of invalid argument
     *
     */
     ...
     *      - ESP_FAIL: initialize Ethernet MAC failed because some other error
     * occurred
     *
     */
     ...
     *      - ESP_FAIL: deinitialize Ethernet MAC failed because some error
     * occurred
     *
     */
     ...
     *      - ESP_FAIL: start Ethernet MAC failed because some other error
     * occurred
     *
     */
     ...
     *      - ESP_ERR_INVALID_ARG: transmit packet failed because of invalid
     * argument
     *      - ESP_ERR_INVALID_STATE: transmit packet failed because of wrong
     * state of MAC
     ...
     *
     * @note Memory of buf is allocated in the Layer2, make sure it get free
     * after process.
     * @note Before this function got invoked, the value of "length" should set
     * by user, equals the size of buffer. After the function returned, the
     * value of "length" means the real length of received data.
     *
     * @return
     *      - ESP_OK: receive packet successfully
     *      - ESP_ERR_INVALID_ARG: receive packet failed because of invalid
     * argument
     *      - ESP_ERR_INVALID_SIZE: input buffer size is not enough to hold the
     * incoming data. in this case, value of returned "length" indicates the
     * real size of incoming data.
     ...
     * argument
     *      - ESP_ERR_INVALID_STATE: read PHY register failed because of wrong
     * state of MAC
     ...
     *
     */
    esp_err_t (*read_phy_reg)(esp_eth_mac_t* mac,
                              uint32_t phy_addr,
                              uint32_t phy_reg,
                              uint32_t* reg_value);
     ...
     *      - ESP_ERR_INVALID_STATE: write PHY register failed because of wrong
     * state of MAC
     ...
     *
     */
    esp_err_t (*write_phy_reg)(esp_eth_mac_t* mac,
                               uint32_t phy_addr,
                               uint32_t phy_reg,
                               uint32_t reg_value);
     ...
     *      - ESP_ERR_INVALID_ARG: set MAC address failed because of invalid
     * argument
     ...
     *      - ESP_ERR_INVALID_ARG: get MAC address failed because of invalid
     * argument
     ...
     *      - ESP_ERR_INVALID_ARG: set MAC speed failed because of invalid
     * argument
     ...
     *      - ESP_ERR_INVALID_ARG: set MAC duplex failed because of invalid
     * argument
     ...
     *      - ESP_ERR_INVALID_ARG: set link status failed because of invalid
     * argument
     ...
     *
     * @param[in] mac: Ethernet MAC instance
     * @param[in] enable: set true to enable promiscuous mode; set false to
     ...
     *
     * @param[in] mac: Ethernet MAC instance
     * @param[in] enable: set true to enable flow control; set false to disable
     * flow control
     *
     * @return
     ...
     *
     * @param[in] mac: Ethernet MAC instance
     * @param[in] ability: zero indicates that pause function is supported by
     * link partner; non-zero indicates that pause function is not supported by
     * link partner
     *
     * @return
     *      - ESP_OK: set peer pause ability successfully
     *      - ESP_FAIL: set peer pause ability failed because some error
     * occurred
     */
     ...
     *      - ESP_FAIL: free Ethernet MAC instance failed because some error
     * occurred
     *
     */
  ...
  {
    /**
     * @brief Default values configured using Kconfig are going to be used when
     * "Default" selected.
     *
     */
    EMAC_CLK_DEFAULT,
    ...
    /**
     * @brief Input RMII Clock from external. EMAC Clock GPIO number needs to be
     * configured when this option is selected.
     *
     * @note MAC will get RMII clock from outside. Note that ESP32 only supports
     * GPIO0 to input the RMII clock.
     *
     */
    EMAC_CLK_EXT_IN,

    /**
     * @brief Output RMII Clock from internal APLL Clock. EMAC Clock GPIO number
     * needs to be configured when this option is selected.
     *
     */
    EMAC_CLK_OUT
  ...
  typedef enum
  {
     ...
     *
     * @note GPIO0 can be set to output a pre-divided PLL clock (test only!).
     * Enabling this option will configure GPIO0 to output a 50MHz clock. In
     * fact this clock doesn’t have directly relationship with EMAC peripheral.
     * Sometimes this clock won’t work well with your PHY chip. You might need
     * to add some extra devices after GPIO0 (e.g. inverter). Note that
     * outputting RMII clock on GPIO0 is an experimental practice. If you want
     * the Ethernet to work with WiFi, don’t select GPIO0 output mode for
     * stability.
     *
     */
    ...
    /**
     * @brief Inverted Output RMII Clock from internal APLL Clock available at
     * GPIO17
     *
     */
  ...
  {
    uint32_t
      sw_reset_timeout_ms;       /*!< Software reset timeout value (Unit: ms) */
    uint32_t rx_task_stack_size; /*!< Stack size of the receive task */
    uint32_t rx_task_prio;       /*!< Priority of the receive task */
    int smi_mdc_gpio_num;  /*!< SMI MDC GPIO number, set to -1 could bypass the
                              SMI  GPIO configuration */
    int smi_mdio_gpio_num; /*!< SMI MDIO GPIO number, set to -1 could bypass the
                              SMI GPIO configuration */
    uint32_t flags; /*!< Flags that specify extra capability for mac driver */
    eth_data_interface_t
      interface; /*!< EMAC Data interface to PHY (MII/RMII) */
    eth_mac_clock_config_t
      clock_config; /*!< EMAC Interface clock configuration */
  } eth_mac_config_t;

#define ETH_MAC_FLAG_WORK_WITH_CACHE_DISABLE                                   \
  (1 << 0) /*!< MAC driver can work when cache is disabled */
#define ETH_MAC_FLAG_PIN_TO_CORE                                               \
  (1 << 1) /*!< Pin MAC task to the CPU core where driver installation         \
              happened */
...
#define ETH_MAC_DEFAULT_CONFIG()                                               \
  {                                                                            \
    .sw_reset_timeout_ms = 100, .rx_task_stack_size = 2048,                    \
    .rx_task_prio = 15, .smi_mdc_gpio_num = 23, .smi_mdio_gpio_num = 18,       \
    .flags = 0, .interface = EMAC_DATA_INTERFACE_RMII, .clock_config = {       \
      .rmii = { .clock_mode = EMAC_CLK_DEFAULT,                                \
...
#define ETH_DM9051_DEFAULT_CONFIG(spi_device)                                  \
   ...
   *      - NULL: create MAC instance failed because some error occurred
   */
  esp_eth_mac_t* esp_eth_mac_new_dm9051(
    const eth_dm9051_config_t* dm9051_config,
...
#define ETH_KSZ8851SNL_DEFAULT_CONFIG(spi_device)                              \
  {                                                                            \
    .spi_hdl = spi_device, .int_gpio_num = 14,                                 \
  }
  ...
  esp_eth_mac_t* esp_eth_mac_new_ksz8851snl(
    const eth_ksz8851snl_config_t* ksz8851snl_config,
    const eth_mac_config_t* mac_config);
