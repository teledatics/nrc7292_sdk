...
extern "C"
{
   ...
   *                      Not saved by the library, can be allocated on the
   * stack.
   * @param[out] out_handle  Output, pointer to esp_timer_handle_t variable
   * which will hold the created timer handle.
   *
   * @return
   *      - ESP_OK on success
   ...
   * @param timeout_us timer timeout, in microseconds relative to the current
   * moment
   * @return
   *      - ESP_OK on success
   ...
   *
   * Timer should not be running when this function is called. This function
   * will start the timer which will trigger every 'period' microseconds.
   *
   * @param timer timer handle created using esp_timer_create
   ...
   *
   * The timer must be stopped before deleting. A one-shot timer which has
   * expired does not need to be stopped.
   *
   ...
   *         The timebase is the same as for the values returned by
   * esp_timer_get_time.
   */
  ...
  /**
   * @brief Get the timestamp when the next timeout is expected to occur
   * skipping those which have skip_unhandled_events flag
   * @return Timestamp of the nearest timer event, in microseconds.
   *         The timebase is the same as for the values returned by
   * esp_timer_get_time.
   */
   ...
   *
   * @note The timeout period is the time interval with which a timer restarts
   * after expiry. For one-shot timers, the period is 0 as there is no
   * periodicity associated with such timers.
   *
   * @param timer timer handle allocated using esp_timer_create
   ...
   *
   * @note This API returns a valid expiry time only for a one-shot timer. It
   * returns an error if the timer handle passed to the function is for a
   * periodic timer.
   *
   * @param timer timer handle allocated using esp_timer_create
   ...
   */
  esp_err_t esp_timer_get_expiry_time(esp_timer_handle_t timer,
                                      uint64_t* expiry);

  /**
   * @brief Dump the list of timers to a stream
   *
   * If CONFIG_ESP_TIMER_PROFILING option is enabled, this prints the list of
   * all the existing timers. Otherwise, only the list active timers is printed.
   *
   * The format is:
   *
   ...
   * where:
   *
   * name — timer name (if CONFIG_ESP_TIMER_PROFILING is defined), or timer
   * pointer period — period of timer, in microseconds, or 0 for one-shot timer
   * alarm - time of the next alarm, in microseconds since boot, or 0 if the
   ...
   * total_callback_run_time - total time taken by callback to execute, across
   * all calls
   *
   ...
   * The context switch will be called after all ISR dispatch timers have been
   * processed.
   */
